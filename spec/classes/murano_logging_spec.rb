...
describe 'murano::logging' do
  ...
  let :log_params do
    {
        ...
        :log_config_append             => '/etc/murano/logging.conf',
        :publish_errors                => true,
        :default_log_levels            => {
            'amqp'       => 'WARN',
            'amqplib'    => 'WARN',
  ...
  shared_examples_for 'murano-logging' do
  ...
  shared_examples_for 'basic logging options passed' do
    context 'with passed params' do
      ...
      it { is_expected.to contain_murano_config('DEFAULT/log_dir').with_value('/tmp/murano') }
    end
  end

  shared_examples_for 'logging params set' do
    it 'enables logging params' do
      is_expected.to contain_murano_config('DEFAULT/logging_context_format_string').with_value(
                         '%(asctime)s.%(msecs)03d %(process)d %(levelname)s %(name)s [%(request_id)s %(user_identity)s] %(instance)s%(message)s')

      is_expected.to contain_murano_config('DEFAULT/logging_default_format_string').with_value(
                         '%(asctime)s.%(msecs)03d %(process)d %(levelname)s %(name)s [-] %(instance)s%(message)s')

      is_expected.to contain_murano_config('DEFAULT/logging_debug_format_suffix').with_value(
                         '%(funcName)s %(pathname)s:%(lineno)d')

      is_expected.to contain_murano_config('DEFAULT/logging_exception_prefix').with_value(
                         '%(asctime)s.%(msecs)03d %(process)d TRACE %(name)s %(instance)s')

      is_expected.to contain_murano_config('DEFAULT/log_config_append').with_value('/etc/murano/logging.conf')
      is_expected.to contain_murano_config('DEFAULT/publish_errors').with_value(true)
      is_expected.to contain_murano_config('DEFAULT/default_log_levels').with_value(
                         'amqp=WARN,amqplib=WARN,boto=WARN,iso8601=WARN,requests.packages.urllib3.connectionpool=WARN,sqlalchemy=WARN,suds=INFO')

      is_expected.to contain_murano_config('DEFAULT/fatal_deprecations').with_value(true)
      is_expected.to contain_murano_config('DEFAULT/instance_format').with_value('[instance: %(uuid)s] ')
      is_expected.to contain_murano_config('DEFAULT/instance_uuid_format').with_value('[instance: %(uuid)s] ')
      is_expected.to contain_murano_config('DEFAULT/log_date_format').with_value('%Y-%m-%d %H:%M:%S')
    end
  end
